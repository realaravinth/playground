...
impl Handler<Visitor> for StateManager {
    type Result = usize;
...
impl Handler<DeleteVisitor> for StateManager {
    type Result = ();
...
mod tests {
    ...
    async fn race(addr: Addr<StateManager>, count: Levels) {
    ...
    async fn counter_focus_works() {
        ...
        let addr = StateManager::default().start();
